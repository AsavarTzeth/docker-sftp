...
if [ $CHECK1 -eq 0 -o $CHECK2 -eq 0 ]; then
	...
	echo >&2 ' If neither of these are true, the instance may not work properly (no sftp login).'
else
	useradd -Ud /files -u $SFTP_UID -s /usr/sbin/nologin -G sftpusers $SFTP_USER
	mkdir -p $SFTP_HOME && chgrp sftpusers $SFTP_HOME
	chmod 550 $SFTP_CHROOT && chmod 775 $SFTP_HOME
	umask 002 $SFTP_HOME && chmod g+s $SFTP_HOME
